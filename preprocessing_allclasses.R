...
# edit 08/07/2022: full compositions 
# 0-4 10-14 15-19 20-24 25-29 30-34 35-39 40-44 45-49 5-9 50-54 55-59 60-64 65-69 70-74 
# 75-79 80-84 85+
################################################################################
...
# years 1959-2015
deaths_who$AgeGroup = deaths_who$Age
deaths_who$AgeGroup[deaths_who$Age %in% c("0", "1", "2", "3", "4")] = "0-4"
deaths_who$AgeGroup[deaths_who$Age %in% c("85-89", "90-94", "95+")] = "85+"
...
deaths_hcod$AgeGroup = deaths_hcod$Age
deaths_hcod$AgeGroup[deaths_hcod$Age %in% c("0", "1-4")] = "0-4"
...
x = data.frame(table(check$CauseM, check$AgeGroup, check$Sex, check$Country)/n_years)
x2 = x %>% group_by(CauseM, AgeGroup, Sex) %>% 
  summarise(p = sum(Freq > 0.1))
# zero counts for more then 10% of the time period in more then one country 
x2 = x2[x2$p > 1,]
exc = unique(interaction(x2$CauseM, x2$AgeGroup))
deaths2$tmp = interaction(deaths2$CauseM, deaths2$AgeGroup)
deaths3 = deaths2 %>% 
  filter(!tmp%in%exc) %>% select(-tmp) %>%
...
save(Z_M, Z_F, X_M, X_F, Y_M, Y_F, deaths3, deaths2, causes, countries, range_years,
     file = "data_fullCOD_fullComp.RData")
